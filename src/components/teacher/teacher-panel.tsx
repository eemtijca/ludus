...
/**
 * TeacherPanel — modo professor: guia pedagógico completo.
 *
 * - Como usar em sala (roteiro sugerido de 50 min)
 * - Fichas dos 12 jogos: BNCC, objetivo, habilidades, duração
...
const LESSON_STEPS = [
  ...
  },
  {
    ...
    text: "Cada jogo pede 8–12 minutos e tem 3 casos. O ideal é 1 caso por sessão para AEE: conclua, celebre o selo e pare — repetir vale mais que avançar.",
  },
  {
    icon: Volume2,
    title: "Voz e leitura",
...
export function TeacherPanel() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      {/* Cabeçalho */}
      <header className="rounded-3xl border-2 border-border bg-white p-6 shadow-[0_5px_0_#e9e2d2] sm:p-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-ink-soft">
              Guia pedagógico dos doze jogos — BNCC, roteiro de uso e
              acompanhamento.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {LESSON_STEPS.map(({ icon: Icon, title, text }) => (
            <div
              key={title}
              className="flex items-start gap-3 rounded-2xl bg-cloud/70 p-4"
            >
              <span
                className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-white text-ink shadow-[0_3px_0_#e9e2d2]"
                aria-hidden
              >
        ...
        <div className="mt-4 flex items-start gap-3 rounded-2xl border-2 border-linguagens/30 bg-linguagens-soft p-4">
          <ShieldCheck
            className="mt-0.5 size-5 shrink-0 text-linguagens-dark"
            aria-hidden
          />
          <p className="text-sm leading-relaxed text-ink-soft">
            <strong className="text-ink">Privacidade por desenho:</strong> todo
            o progresso fica no próprio dispositivo (localStorage), sem conta,
            sem servidor e sem dado pessoal. Para acompanhar um estudante
            específico, use o mesmo dispositivo da sessão — ou peça que ele
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-3">
                {games.map((game) => {
                  ...
                  return (
                    <article
                      key={game.id}
                      className="flex flex-col gap-3 rounded-3xl border-2 border-border bg-white p-5 shadow-[0_5px_0_#e9e2d2]"
                    >
