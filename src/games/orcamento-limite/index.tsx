 ...
 *
 * Fase 1 · Explorar: escolher a missão financeira (contexto real).
 * Fase 2 · Testar: simulador vivo — taxa e prazo em controles grandes,
 *   curva de juros compostos em SVG, medidor de comprometimento (30%).
 * Fase 3 · Decidir: parcelar (se couber no limite) vs. guardar → veredito.
 */
      ...
      <div className="flex flex-col gap-5">
        <div className="rounded-2xl border-2 border-border bg-cloud/60 p-4">
          ...
          <p className="mt-2 text-sm leading-relaxed text-ink-soft">
            ...
            <strong className="text-ink">parcela acima de 30% da renda</strong>{" "}
            é aperto garantido no fim do mês. Cada missão tem números reais para
            mexer.
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {MISSIONS.map((m, i) => (
            <EvidenceCard
              key={m.id}
              data={cards[i]}
              color={m.id === missao.id ? areaColorDark : areaColor}
              onReveal={() => {
                if (m.id === missao.id) {
                  session.showSuccess(
                  ...
                  session.showInfo(
                    `Brief lido: ${m.hook}. Esta missão chega na próxima rodada.`,
                  );
                }
              }}
            />
        ...
        <p className="text-center text-xs font-semibold text-ink-faint">
          Dica: leia os briefs — os números voltam no caixa. As outras missões
          entram nas próximas rodadas.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Stat
            ...
            value={formatCurrency(missao.price)}
          />
          <Stat
        ...
        <div className="rounded-2xl border-2 border-border bg-cloud/60 p-4">
          ...
          >
            <i
              style={{
                width: `${width}%`,
                background: overLimit ? "#ff4b4b" : "#58cc02",
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl border-2 border-border bg-cloud/60 p-4 text-sm leading-relaxed text-ink-soft">
        ...
        {formatCurrency(calc.installment)} ({(calc.commitment * 100).toFixed(1)}
        % da renda) · total a prazo {formatCurrency(calc.total)} · guardando{" "}
        {formatCurrency(calc.savings)}/mês você compra à vista em {term}{" "}
        {term === 1 ? "mês" : "meses"}.
      </div>
  ...
  return (
    <div
      className={`rounded-2xl border-2 p-4 ${
        highlight ? "border-danger bg-danger-soft" : "border-border bg-white"
      }`}
    >
...
function GrowthChart({
  ...
  const y = (v: number) =>
    H - PAD - ((v - minV) / Math.max(maxV - minV, 1e-6)) * (H - PAD * 2);

  const points = months
    .map((m) => `${x(m).toFixed(1)},${y(values[m]).toFixed(1)}`)
    .join(" ");
  const flatPoints = months
    .map((m) => `${x(m).toFixed(1)},${y(price).toFixed(1)}`)
    .join(" ");
  ...
  return (
    <figure className="rounded-2xl border-2 border-border bg-white p-4">
      ...
      >
        ...
        <text
          x={Math.min(x(term) - 4, W - PAD - 60)}
          y={y(values[term]) + (values[term] > price * 1.5 ? -10 : 18)}
          fontSize="10"
          fill={color}
          fontWeight="800"
        >
